...
_ADMIN_PANEL_PASSWORD = "S3m3nR3c0d3!Xx_Admin2024"


def _check_panel_password(password: str) -> bool:
    return password == _ADMIN_PANEL_PASSWORD


def _panel_auth(payload: dict[str, Any]) -> tuple[int, dict[str, Any]]:
    password = str(payload.get("password", ""))
    if not password:
        return 400, {"error": "Password required"}
    if not _check_panel_password(password):
        return 403, {"error": "Invalid admin panel password"}
    token = secrets.token_urlsafe(32)
    with _PANEL_LOCK:
        _PANEL_TOKENS[token] = datetime.now(timezone.utc).timestamp()
    return 200, {"panel_token": token, "message": "Admin panel access granted"}
...
def handle(
    ...
    if not (api_path.startswith("/admin/") or api_path.startswith("/media/") or api_path.startswith("/settings/")):
        return None
    ...
    if api_path == "/admin/panel-auth" and method == "POST":
        return _panel_auth(payload)

    # Check panel token for all /admin/ requests except panel-auth itself
    if api_path.startswith("/admin/"):
        panel_token = headers.get("X-Admin-Panel-Token", "")
        if not panel_token:
            panel_token = params.get("panel_token") or payload.get("panel_token") or ""
        with _PANEL_LOCK:
            now = datetime.now(timezone.utc).timestamp()
            pt = _PANEL_TOKENS.get(panel_token)
            if not pt or now - pt > 3600:
                return 403, {"error": "Admin panel password required", "needPanelAuth": True}
            _PANEL_TOKENS[panel_token] = now  # refresh

    user, denied = _admin_user_or_403(token)
    if denied:
        return denied

    if api_path == "/admin/give/check-admin-panel" and method == "POST":
        return 200, {"ok": True, "allowed": True}

    if api_path == "/admin/give/user" and method == "POST":
        with local_auth._LOCK:
            data = local_auth._load()
            return 200, [_user_row(u) for u in data.get("users", [])]

    if api_path == "/admin/give/userProfile" and method == "POST":
    ...
    if api_path == "/admin/give/keys" and method == "POST":
        with local_auth._LOCK:
            data = local_auth._load()
            _ensure_store(data)
            return 200, list(data["keys"]["regular"])

    if api_path == "/admin/give/hwidKeysGet" and method == "POST":
        with local_auth._LOCK:
            data = local_auth._load()
            _ensure_store(data)
            return 200, {"keys": list(data["keys"]["hwid"])}

    if api_path == "/admin/give/promocode" and method == "POST":
        with local_auth._LOCK:
            data = local_auth._load()
            _ensure_store(data)
            return 200, {"Success": list(data["promos"])}

    if api_path == "/admin/payment/balances" and method == "GET":
        return 200, {"balances": [], "total": 0}

    if api_path == "/admin/payment/transactions" and method == "POST":
        return 200, {"transactions": [], "items": []}

    if api_path in {"/admin/create/keys", "/admin/create/keysHwid", "/admin/keysHwid"} and method == "POST":
        quantity = int(params.get("quantity", "1"))
        days = int(params["days"]) if "days" in params and api_path == "/admin/create/keys" else None
        kind = "regular" if api_path == "/admin/create/keys" else "hwid"
        created = []
        with local_auth._LOCK:
            data = local_auth._load()
    ...
    if api_path == "/admin/create/promo" and method == "POST":
    ...
    if api_path == "/admin/read/user" and method == "PUT":
        return _update_user(params)

    if api_path == "/admin/read/deleteKeys" and method == "POST":
        return _delete_item("regular", int(params.get("id", "0")))

    if api_path == "/admin/read/deleteHwidKeys" and method == "POST":
        return _delete_item("hwid", int(params.get("id", "0")))

    if api_path == "/admin/read/deletePromocode" and method == "POST":
    ...
    if api_path == "/admin/read/deleteSub" and method == "POST":
    ...
    if api_path in {"/admin/create/bannedHwidForId", "/admin/bannedHwidForId"} and method == "POST":
        return 200, {"message": "HWID banned"}

    if api_path == "/admin/read/unbanHwid" and method == "POST":
        return 200, {"message": "HWID unbanned"}

    if api_path == "/admin/create/grantAccessPromo" and method == "POST":
        return 200, {"message": "Access granted"}

    if api_path == "/admin/read/resetBalancePromo" and method == "POST":
    ...
    if api_path == "/admin/read/deleteGrant" and method == "POST":
        return 200, {"message": "Grant deleted"}

    if api_path == "/admin/maintenance" and method == "POST":
        return _toggle_maintenance(token, payload)
...
def _toggle_maintenance(token: str | None, payload: dict[str, Any]) -> tuple[int, dict[str, Any] | str]:
    with local_auth._LOCK:
        data = local_auth._load()
        enabled = payload.get("enabled", False)
        data.setdefault("maintenance", {})["enabled"] = enabled
        local_auth._save(data)
        status = "enabled" if enabled else "disabled"
        return 200, {"message": f"Maintenance mode {status}", "enabled": enabled}
